...
int compare_strings(char *a, char *b) {
   ...
   if (aux == strlen(b)-2) return 1;

   return 0;
}

void split_read(char *a, char *b, char *arg){   

   if(strchr(b,' ')==NULL) strcpy(a,b);
	...
	for (;;) {
      ...
      if ( (childpid = fork()) == 0) {
         ...
         while (state == 0){
               ...
               if (substate == 1) {
                  if ((n=read(connfd, recvline, MAXLINE)) > 0) {
                     ...
                     for (int a = 0; a < USERSSIZE; a++) {
                        ...
                        for (int j=0; j< strlen(recvline); j++){
                           if (recvline[j] == s[j]) contlen++; 
                        }  
            ...
            msg = "331 Password required for matheus\n";
            ...
            if (substate == 2) {
               ...
               if ((n=read(connfd, recvline, MAXLINE)) > 0) {
                  ...
                  if (position != -1) {
                     ...
                     s = passwords[position];

                     for (int a = 0; a < strlen(passwords[position]); a++) {
                        if (recvline[a] == s[a]) contlen++;
                     }

                     if (contlen == strlen(passwords[position])) msg = "230 User matheus logged in\n";

                     if (contlen != strlen(passwords[position])) msg = "530 Login incorrect\n";
         ...
         while ((n=read(connfd, recvline, MAXLINE)) > 0) {
            ...
            split_read(comand,recvline,arg);

            printf("%s, %s\n", comand, arg);

            // write(connfd, recvline, strlen(recvline));

            // memset(comand, '0', MAXLINE + 1);
         }
         /* ========================================================= */
         /* ========================================================= */
         /*                         EP1 FIM                           */
